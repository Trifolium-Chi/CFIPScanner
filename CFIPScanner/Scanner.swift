...
final class Scanner: ObservableObject {
    ...
    // 地区实时检测：记录已检测/检测中的 IP，避免重复请求
    private var detectedIPs: Set<String> = []
    private var detectingIPs: Set<String> = []
    ...
    /// 扫描进行中实时检测某个 IP 的真实地区（去重、失败不重试，避免重复请求）
    private func detectRegion(for ip: String) {
        lock.lock()
        if detectedIPs.contains(ip) || detectingIPs.contains(ip) {
            lock.unlock()
            return
        }
        detectingIPs.insert(ip)
        lock.unlock()

        geoLookup(ip: ip) { [weak self] geo in
            DispatchQueue.main.async {
                guard let self = self else { return }
                self.lock.lock()
                self.detectingIPs.remove(ip)
                self.detectedIPs.insert(ip)
                self.lock.unlock()
                if let geo = geo, !geo.countryCode.isEmpty {
                    if let idx = self.results.firstIndex(where: { $0.ip == ip }) {
                        self.results[idx].cc = geo.countryCode
                        self.results[idx].region = regionName(geo.countryCode)
                    }
                }
                self.refreshOutputText()
            }
        }
    }
    ...
    func startSpeedJob(ip: String, port: Int) -> String {
        ...
        DispatchQueue.global(qos: .userInitiated).async { [weak self] in
            let dl = downloadSpeed(ip: ip, port: port) { total, elapsed in
                let speed = elapsed > 0 ? (Double(total) / 1000.0 / 1000.0) / elapsed : 0.0
